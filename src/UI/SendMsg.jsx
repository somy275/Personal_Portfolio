...
import { notifyOnError, notifyOnSuccess, notifyPending } from './Notifications';
...
const SendMsg = () => {
    ...
    const onSendEmail = async(data) => {
        if (window.navigator.onLine) {
            const EmailResponse= axios.post("http://localhost:8585/sendEmail",data)
            notifyPending(EmailResponse)
            try{
              const res= await EmailResponse;
    ...
    return (
        <div ref={inputText} className='translate-x-[400%] w-full min-[450px]:w-[max(20rem,65vw)] md:w-[max(25rem,60vw)] max-w-[280px] min-[450px]:max-w-[400px] md:max-w-[500px] lg:max-w-none lg:w-[40%] h-auto flex flex-col items-center text-[#e8eef1]'>
            ...
            <form ref={ref} onSubmit={handleSubmit(onSendEmail)} className=' w-full flex flex-col justify-around gap-y-[3rem] py-[1.8rem]'>
